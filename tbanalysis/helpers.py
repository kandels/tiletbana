...
def mask_array_threshold(arr: np.ndarray, threshold: float = 0) -> np.ma.MaskedArray:
    """
...
def reco_energy(h1000: np.ndarray, modules:List[str]=None, events: list = None, cell: str = None, data_type='data', return_indices=False):
    """
    ...
    if modules is None:
        modules = ['LBA65', 'LBC65', 'EBC65', 'M0C']  # Default modules. M0A should not be used for 2023 onwards TB analysis
    ...
    if cell is None:
        ...
        lba_chan_ene = mask_array_threshold(lba_chan_ene, threshold=threshold)
        lbc_chan_ene = mask_array_threshold(lbc_chan_ene, threshold=threshold)
        ebc_chan_ene = mask_array_threshold(ebc_chan_ene, threshold=threshold)
        mzeroc_chan_ene = mask_array_threshold(mzeroc_chan_ene, threshold=threshold)
        ...
        for mod in modules:
            if mod == 'LBC65':
                ...
                mod_energy = mask_array_threshold(mod_energy, threshold=threshold)
...
class ParticleSeparator:
    ...
    def __init__(self, h1000, data_type='data'):
        ...
        self.lba_energy = mask_array_threshold(lba_energy[self.lb_cells])
        self.lbc_energy = mask_array_threshold(lbc_energy[self.lb_cells])
        self.ebc_energy = mask_array_threshold(ebc_energy[self.eb_cells])
        self.mzeroc_energy = mask_array_threshold(mzeroc_energy[self.lb_cells])
    ...
    def muon_events(self,  h1000=None) -> np.ndarray:
        ...
        lba_energy = mask_array_threshold(lba_energy)
        lbc_energy = mask_array_threshold(lbc_energy)
        ebc_energy = mask_array_threshold(ebc_energy)
        mzeroc_energy = mask_array_threshold(mzeroc_energy)
        ...
        tot_energy = tot_energy.filled(np.inf)
    ...
    def alter_muon_rejection(self, h1000=None) -> np.ndarray:
        ...
        and deposit energy in the outer radial D-layer. Electrons and hadrons typically
        ...
        lb_d1_cells = [13, 14] #LB D cells
        lb_d2_cells = [25, 26]
        lb_d3_cells = [39, 42]
        
        eb_d5_cells = [16, 17] #EB D cells
        eb_d6_cells = [30, 31]
        
        sigma_noise = 30  # MeV. Electronic noise
        threshold = 2 * sigma_noise
